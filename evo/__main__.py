...
import numpy as np
import pygame
import argparse, os, time, csv, imageio
...
def parse_args():
    p = argparse.ArgumentParser("NEAT Coevolution Simulation")
    p.add_argument("--steps", type=int, default=5000)
    p.add_argument("--save-interval", type=int, default=10,
                   help="Save a screenshot every N steps")
    p.add_argument("--output-dir", type=str, default="out/evo",
                   help="Directory for snapshots and metrics")
    return p.parse_args()
...
def main():
    ...
    # make output folders & open metrics CSV
    ...
    writer.writerow(["t", "grass_pop", "prey_pop"])
    ...
    while running:
        ...
        # --- log metrics ---
        writer.writerow([t, len(grasses), len(preys)])

        # --- save frame every save_interval timesteps ---
        if t % args.save_interval == 0 or t + 1 == args.steps:
            fname = os.path.join(frames_dir, f"frame_{t:05d}.png")
